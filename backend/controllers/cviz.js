...
class CvizSlot {
  ...
  buildClientState() {
    ...
    if (
      this.lastState.timelineFile != this.templateName &&
      this.lastState.state.toLowerCase() != "clear"
    ) {
      this.adjustmentList = [];
...
client.on("data", (data) => {
  try {
    ...
    if (
      blob.timelineSlot == "lowerthird" &&
      !state.isRunningAnything() &&
      state.adjustmentList.length > 0
    ) {
...
export function setup(Models, app) {
  ...
  app.delete("/api/cviz/:slot/adjustment/:id", (req, res) => {
    ...
    state.adjustmentList = state.adjustmentList.filter(
  ...
  app.post("/api/cviz/:slot/cue", (req, res) => {
    ...
    if (
      (state.lastState.state || "").toLowerCase() == "cueorchild" &&
      state.adjustmentList.length > 0
    ) {
  ...
  app.post("/api/run/lowerthird", (req, res) => {
    ...
    const type = req.body.headline
      ? "GE2018/LT-ANI-HEADLINE"
      : "GE2018/LT-ANI-GREY";
  ...
  app.post("/api/run/lowerthird/:id", (req, res) => {
    ...
    return Person.findById(req.params.id, {
      ...
      .then((data) => {
        ...
        const suffix =
          data.Position.type.indexOf("candidate") == 0
            ? data.elected
              ? " Elect"
              : " Candidate"
            : "";
  ...
  app.post("/api/run/person/:id/:template", (req, res) => {
    ...
    return Person.findById(req.params.id, {
      ...
      .then((data) => {
        ...
        if (
          type.toLowerCase() == "sidebarphoto" ||
          type.toLowerCase() == "sidebartext"
        )
          type = "sidebar";
    ...
    switch (req.params.template.toLowerCase()) {
      case "winnersall":
        return getWinnersOfType(Models, "candidateSabb")
          .then(function (sabbs) {
            return getWinnersOfType(Models, "candidateNonSabb").then(function (
              nonsabbs
            ) {
              const data = splitWinnersIntoPages(
                "Part-time Officer Elects",
                nonsabbs
              );
              const data2 = splitWinnersIntoPages(
                "Full-time Officer Elects",
                sabbs
              );
          ...
          .then(function (position) {
            ...
            if (people.length > 8) {
              while (people.length > 0) {
                ...
                data.push([
                  position.fullName,
                  buildCandidateForPosition(position, removed),
                ]);
              }
            } else {
              data.push([
                position.fullName,
                buildCandidateForPosition(position, people),
              ]);
            }
